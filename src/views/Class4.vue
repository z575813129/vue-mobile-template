...
<template>
  <div>
    ...
    <div class="v-class4" v-if="!isLoading">
      <list :source-i-d="sourceID" :college-i-d="collegeID" :list-style="instance.source.listStyle"></list>
      <div v-if="sourceID===''||!sourceID" class="spinner">数据暂无</div>
    </div>
  </div>
</template>
...
<script>
  ...
  import List from '../components/Class4'
  ...
  export default{
    ...
    computed: {
      ...
      sourceID () {
        if (this.instance && this.instance.source && this.instance.source.ID) {
          return this.instance.source.ID
        }
      }
    }
  }
